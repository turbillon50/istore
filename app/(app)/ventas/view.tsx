...
export default function VentasPage({ posCatalog }: { posCatalog: { id: string; name: string; price: number; category: string }[] }) {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      if (!res.ok || !d.ok) throw new Error(d.error ?? "Error de cobro");
      toast({
        title: `Venta cobrada · ${formatCurrency(total)}`,
        description: `Ticket #V-${d.sale.id} · ${method} · ${cart.length} artículos`,
        tone: "success",
      });
      setCart([]);
    } catch (e) {
      toast({ title: "No se pudo cobrar", description: String(e), tone: "destructive" as any });
    } finally {
      setPaying(false);
    }
  };
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="flex h-fit flex-col lg:sticky lg:top-20">
          ...
          <CardContent className="space-y-3">
            ...
            <Button className="w-full" size="lg" disabled={cart.length === 0 || paying} onClick={checkout}>
